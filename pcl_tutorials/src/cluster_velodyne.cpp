...
double getClusterInfo(CloudA pt, PointA &cluster)
{
	...
	cluster.normal_x = fabs(max_p[0]-min_p[0]);
	cluster.normal_y = fabs(max_p[1]-min_p[1]);
	// cluster.normal_z = fabs(max_p[2]);
	cluster.normal_z = fabs(max_p[2]-min_p[2]);
...
int main (int argc, char** argv)
{
	...
	ros::Publisher dynamic_centroid_pub = n.advertise<sensor_msgs::PointCloud2>("/dynamic/centroid",1); // 動的物体と思われるクラスタの重心
